...
public class RollingBallPuzzle : MonoBehaviour
{
    ...
    [SerializeField] private Interactable joystickInteractable;
    ...
    [SerializeField] private GameObject ballPrize;

    private void Awake() {
        ballRigidbody = ball.GetComponent<Rigidbody>();
        ballOriginalPosition = ball.transform.localPosition;

        if (Settings.load && SaveSystem.CheckFlag("rolling_ball_puzzle_won")) {
            ballPrize.SetActive(true);
            Destroy(ball);
            joystickInteractable.DisableInteraction();
        }
        else {
            ball.SetActive(true);
        }
    ...
    void Update()
    {
        if (won || !puzzleCamera.activeSelf)
            return;
    ...
    public IEnumerator Win() {
        SaveSystem.SetFlag("rolling_ball_puzzle_won");
        won = true;
